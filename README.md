...
| Live zoom | `z` steps 2×, 3×, 4×; `0` resets (GNOME's magnifier) | `z` or **Ctrl+Alt+Z** steps 2×, 3×, 4×; `0` or **Ctrl+Alt+0** resets (Windows' Magnification API), seen working ([E018](docs/evidence/E018-windows-live-zoom.md)) | `z` or **Control+Option+Z** steps 2×, 3×, 4×; `0` or **Control+Option+0** resets. yappyink draws the zoom itself with ScreenCaptureKit, so macOS asks for **Screen Recording** permission once. The ink is not magnified yet. **New, not yet seen working** |
...
| <img src="docs/images/toolbar/zoom.png" width="34" alt="Zoom"> | Zoom | `z`, `0` off | Live zoom 2×, 3×, 4×, off, following the pointer | **Ubuntu** (GNOME's magnifier; your settings are restored afterwards) and **Windows** (Magnification API, also Ctrl+Alt+Z / Ctrl+Alt+0 from anywhere): the system magnifies, so no screen pixels reach yappyink. **macOS** (Control+Option+Z / Control+Option+0): yappyink captures and magnifies the screen itself, which needs Screen Recording permission; the ink is not magnified yet, and this is not yet seen working |
