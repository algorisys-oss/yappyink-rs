...
**Opening floating, 2026-09-25.** On a 1366x768 screen GNOME auto-maximized the overlay every launch (`org.gnome.mutter auto-maximize`): the 1280x720 request was 97% of the work area, and a maximized window has Always on Top and resizing disabled (E002 finding 3), so the owner had to restore, pin and resize each time. The adapter now reads the outputs before the first frame and asks for at most 80% of each side of the smallest (`sizing.rs`, 1092x614 here), and asks once to un-maximize if GNOME maximizes it anyway. Observed on the E001 machine: first configure 1092x614, not maximized. Always on Top is still one step per launch; only the Shell extension removes it. The size is now remembered (`window-size` beside the session file). A floating window's configure size is a suggestion under xdg-shell, and the adapter declines GNOME's in favour of the user's size, following the compositor only when maximized, fullscreen, tiled or mid-resize; three earlier designs failed (learning §23). Keeping 1366x697 held the size but not the place: the window sat where GNOME had centred a 1024x522 one and ran off screen, because a Wayland client cannot position itself. So a remembered size is capped at the same 80% of each side as the default, which GNOME places correctly; smaller sizes are restored exactly. A full-screen size on launch needs the Shell extension.
...
**Built, 2026-09-25.** `ink-platform-wayland/src/magnifier.rs` drives the Shell magnifier through `gsettings` on a worker thread; the controller steps 2x, 3x, 4x and off (`Action::CycleZoom`, `ZoomOff`) from `z`, a toolbar button shown only where a magnifier was found, and `yappyink zoom` / `zoom-off`. The user's values go to a restore file before the first change and come back on zoom-off, on exit, and on the next launch after a kill; all three were checked live (E017). `doctor` reports `live_zoom`. Windows and macOS report it `unknown` with their task numbers and answer `z` with a message. The owner then pressed `z`, `0` and the zoom button and confirmed all three, after a fix routing Wayland keys through the shared keymap. Remaining for the exit criterion: a video observed staying live while magnified.
