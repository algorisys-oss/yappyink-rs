...
**Remaining, and why the exit criterion is not met:** none of this has been tried against a live input method, so there is no evidence that composing Hindi or CJK produces the right text on screen. `fontdue` does no shaping or font fallback either, so a script needing reordering or a glyph outside the loaded face will not render correctly even when the text is right. There is also no caret movement within a run, no selection inside text, and no re-editing of a committed text object.
